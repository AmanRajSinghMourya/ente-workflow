...
4. Only if no unresolved behavior/support/maintenance decision, update the four pins
   and relevant shared lock entry; refresh three Podfile.lock checksums as required.
...
5. Run frozen-pubspec check, flutter pub get --enforce-lockfile, relevant format/analyze
   and affected tests per mobile-lint.yml; verify three pod install --deployment runs
...
- CocoaPods remains buildable and the three lockfiles reproduce in deployment mode.
