...
Existing Zoho server configured; waiting for browser OAuth authorization. No tickets retrieved.
...
> Subsequent user message supplied an existing server URL and explicitly authorized adding it to Codex, deferring ticket workflow decisions until later this week. Appended only mcp_servers.zoho-desk to personal config (mode 0600). MCP initialize returned HTTP 401 with OAuth challenge; codex mcp login opened Zoho authorization. Credentials/URL are deliberately omitted from records.
