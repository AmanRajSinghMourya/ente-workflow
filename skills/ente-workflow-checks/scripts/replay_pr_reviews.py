...
class ReviewCollectorTests(unittest.TestCase):
    ...
    def test_cli_defaults_to_installed_workflow_root(self):
        with patch.object(sys, "argv", ["pr_reviews.py"]), \
                patch.object(pr_reviews, "collect", return_value={"status": "held"}) as collect, \
                patch("builtins.print"):
            self.assertEqual(pr_reviews.main(), 0)
        collect.assert_called_once_with(Path(pr_reviews.__file__).resolve().parents[3])

    def opt_in(self):
        (self.root / ".workflow").mkdir(exist_ok=True)
        self.write(".workflow/repository.json", {"repository": "AmanRajSinghMourya/ente-workflow"})

    def assert_storage_rejected(self):
        result = self.run_collection()
        self.assertEqual(result["status"], "incomplete", result)
        self.assertEqual(self.api.calls, [])
        self.assertFalse((self.root / "raw").exists())
        self.assertFalse((self.root / "review-findings.jsonl").exists())

    def test_opted_in_workflow_git_root_allows_collection_and_disposition(self):
        (self.root / ".git").mkdir()
        self.opt_in()
        result = self.run_collection()
        self.assertEqual(result["status"], "completed", result)
        result = self.classify(self.assessment())
        self.assertEqual(result["status"], "completed", result)
        self.assertEqual(self.findings()[0]["disposition"], "confirmed")

    def test_arbitrary_git_root_is_rejected(self):
        (self.root / ".git").mkdir()
        self.assert_storage_rejected()

    def test_opted_in_root_inside_another_git_repository_is_rejected(self):
        (self.root / ".git").mkdir()
        ...
        self.opt_in()
        self.write("tracked-prs.json", self.tracked)
        self.write("control.json", {"not_before": "2026-09-20T12:00:00Z"})
        self.assert_storage_rejected()

    def test_opted_in_git_worktree_file_is_rejected(self):
        (self.root / ".git").write_text("gitdir: /fixture/product/.git/worktrees/task\n")
        self.opt_in()
        self.assert_storage_rejected()

    def test_opted_in_git_directory_symlink_is_rejected(self):
        metadata = self.root / "git-metadata"
        metadata.mkdir()
        (self.root / ".git").symlink_to(metadata, target_is_directory=True)
        self.opt_in()
        self.assert_storage_rejected()

    def test_invalid_workflow_repository_markers_are_rejected(self):
        (self.root / ".git").mkdir()
        self.opt_in()
        invalid = ({}, {"repository": "AmanRajSinghMourya/ente"},
                   {"repository": "AmanRajSinghMourya/ente-workflow", "extra": True},
                   "AmanRajSinghMourya/ente-workflow", None)
        for marker in invalid:
            with self.subTest(marker=marker):
                self.write(".workflow/repository.json", marker)
                self.assert_storage_rejected()
        (self.root / ".workflow/repository.json").write_text("not JSON\n")
        self.assert_storage_rejected()
